...
logger = logging.getLogger("EvaluateMetricsCSV")
...
def load_ground_truth_csv(file_path: str) -> dict:
    ...
    Ожидается, что CSV имеет колонки:
      ...
      - true_items: строка с разделёнными запятыми идентификаторами релевантных объектов.
    Возвращает словарь {user: set(этих объектов)}.
    """
    df = pd.read_csv(file_path)
    # Определяем имя столбца для идентификатора пользователя
    if "user" in df.columns:
        user_col = "user"
    elif "userID" in df.columns:
        user_col = "userID"
    else:
        raise KeyError("В файле ground truth не найден столбец 'user' или 'userID'.")
...
def load_recommendations(file_path: str) -> dict:
    ...
    Ожидается, что CSV имеет колонки:
      ...
      - recommended_items: строка с разделёнными запятыми идентификаторами объектов.
    Возвращает словарь {user: [item1, item2, ...]}.
    """
    df = pd.read_csv(file_path)
    recs = {}
    for _, row in df.iterrows():
        user = row["user"] if "user" in df.columns else row["userID"]
...
def main():
    # Загружаем ground truth из файла "test_ground_truth_restaurant.csv"
    gt_file = "test_ground_truth_restaurant.csv"
    if not os.path.exists(gt_file):
        logger.error("Файл ground truth '%s' не найден!", gt_file)
        return
    ground_truth = load_ground_truth_csv(gt_file)

    # Файлы с рекомендациями (предполагается, что они уже сохранены)
    rec_files = {
        "CAMF": "camf_restaurant_recommendations.csv",
        "CSLIM": "cslim_restaurant_recommendations.csv",
        "LightFM": "lightfm_restaurant_recommendations.csv",
        "SurpriseSVD": "surprise_svd_restaurant_recommendations.csv"
    }
    ...
    print("\nРезультаты MAP@50 по моделям (используя test_ground_truth_restaurant.csv):")
